let Kinlink = {
  ...
  CreateVideoElements: function () {
    ...
    for (var i = 0; i < elements.length; i++) {
      new Kinlink.VideoElement(elements[i])
    }
  },
...
if (document.readyState === "complete") {
  Kinlink.CreateVideoElements()
} else {
  document.addEventListener("DOMContentLoaded", Kinlink.CreateVideoElements)
}
Kinlink.VideoElement = function () {
  "use strict";
  var VideoElement = function (element) {
    ...
    this.player = new Kinlink.Player(url, options);
...
Kinlink.Player = function () {
  ...
  var Player = function (url, options) {
    ...
    if (options.source) {
      ...
      this.source = new Kinlink.Source.WebSocket(url, options);
      options.streaming = true
    } else if (options.progressive !== false) {
      this.source = new Kinlink.Source.AjaxProgressive(url, options);
      options.streaming = false
    } else {
      this.source = new Kinlink.Source.Ajax(url, options);
      options.streaming = false
    }
    this.maxAudioLag = options.maxAudioLag || .25;
    this.loop = options.loop !== false;
    this.autoplay = !!options.autoplay || options.streaming;
    this.demuxer = new Kinlink.Demuxer.TS(options);
    this.source.connect(this.demuxer);
    if (options.video !== false) {
      this.video = new Kinlink.Decoder.MPEG1Video(options);
      this.renderer = !options.disableGl && Kinlink.Renderer.WebGL.IsSupported() ? new Kinlink.Renderer.WebGL(options) : new Kinlink.Renderer.Canvas2D(options);
      this.demuxer.connect(Kinlink.Demuxer.TS.STREAM.VIDEO_1, this.video);
      this.video.connect(this.renderer)
    }
    if (options.audio !== false && Kinlink.AudioOutput.WebAudio.IsSupported()) {
      this.audio = new Kinlink.Decoder.MP2Audio(options);
      this.audioOut = new Kinlink.AudioOutput.WebAudio(options);
      this.demuxer.connect(Kinlink.Demuxer.TS.STREAM.AUDIO_1, this.audio);
  ...
  Player.prototype.seek = function (time) {
    ...
    this.startTime = Kinlink.Now() - time
  };
  ...
  Player.prototype.update = function () {
    ...
    if (!this.isPlaying) {
      this.isPlaying = true;
      this.startTime = Kinlink.Now() - this.currentTime
    }
  ...
  Player.prototype.updateForStaticFile = function () {
    ...
    if (this.audio && this.audio.canPlay) {
      ...
      var targetTime = Kinlink.Now() - this.startTime + this.video.startTime,
...
Kinlink.Source.AjaxProgressive = function () {
  "use strict";
  ...
  AjaxProgressiveSource.prototype.loadNextChunk = function () {
    ...
    this.isLoading = true;
    this.loadStartTime = Kinlink.Now();
    this.request = new XMLHttpRequest;
    this.request.onreadystatechange = function () {
  ...
  AjaxProgressiveSource.prototype.onChunkLoad = function (data) {
    ...
    this.loadTime = Kinlink.Now() - this.loadStartTime;
    if (!this.throttled) {
      this.loadNextChunk()
    }
  };
...
Kinlink.Source.WebSocket = function () {
  "use strict";
...
Kinlink.Demuxer.TS = function () {
  ...
  TS.prototype.write = function (buffer) {
    if (this.leftoverBytes) {
      ...
      this.bits = new Kinlink.BitBuffer(totalLength);
      this.bits.write([this.leftoverBytes, buffer])
    } else {
      this.bits = new Kinlink.BitBuffer(buffer)
    }
  ...
  TS.prototype.resync = function () {
    ...
    console.warn("Kinlink: Possible garbage data. Skipping.");
...
Kinlink.Decoder.MPEG1Video = function () {
  "use strict";
  var MPEG1 = function (options) {
    Kinlink.Decoder.Base.call(this, options);
    var bufferSize = options.videoBufferSize || 512 * 1024;
    var bufferMode = options.streaming ? Kinlink.BitBuffer.MODE.EVICT : Kinlink.BitBuffer.MODE.EXPAND;
    this.bits = new Kinlink.BitBuffer(bufferSize, bufferMode);
  ...
  MPEG1.prototype = Object.create(Kinlink.Decoder.Base.prototype);
  MPEG1.prototype.constructor = MPEG1;
  MPEG1.prototype.write = function (pts, buffers) {
    Kinlink.Decoder.Base.prototype.write.call(this, pts, buffers);
...
Kinlink.Decoder.MP2Audio = function () {
  "use strict";
  ...
  MP2.prototype = Object.create(Kinlink.Decoder.Base.prototype);
...
Kinlink.Renderer.WebGL = function () {
  "use strict";
...
Kinlink.Renderer.Canvas2D = function () {
  "use strict";
  ...
  CanvasRenderer.prototype.resize = function (width, height) {
    ...
    Kinlink.Fill(this.imageData.data, 255)
  };
...
Kinlink.AudioOutput.WebAudio = function () {
  "use strict";
  ...
  WebAudioOut.prototype.play = function (sampleRate, left, right) {
    ...
    if (this.startTime < now) {
      this.startTime = now;
      this.wallclockStartTime = Kinlink.Now()
    }
  ...
  WebAudioOut.prototype.getEnqueuedTime = function () {
    return Math.max(this.wallclockStartTime - Kinlink.Now(), 0)
  };
